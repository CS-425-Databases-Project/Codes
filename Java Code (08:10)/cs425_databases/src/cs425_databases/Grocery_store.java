...
public class Grocery_store {
	...
	private void createOrdersView() {
		...
		String[] columnNames = {"Order No.", "Product", "Qty", "Credit Card Used", "Status", "Time Stamp", "Cancel Order"};
        Object[][] data = {};
        ...
        CancelOrder canceller = new CancelOrder(oTable, 6);
	...
	private ArrayList<ArrayList<Object>> getCustomerDetails() {
		return SQLInterface.sqlExecute("select * from login, customer where uid = cid and cid = '" + userInfo[0]+"';");
	...
	class AddToCartButton extends AbstractCellEditor implements TableCellRenderer, TableCellEditor, ActionListener {
		...
		class CancelOrder extends AbstractCellEditor implements TableCellRenderer, TableCellEditor, ActionListener {
			JTable table;
			JButton renderButton;
			JButton editButton;
			String text;

			public CancelOrder(JTable table, int column) {
				super();
				this.table = table;
				renderButton = new JButton();
			...
			{
				fireEditingStopped();
				cancelOrder(table.getSelectedRow());
			}
