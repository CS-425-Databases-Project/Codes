...
public class CartMenu {
	...
	public ArrayList<ArrayList<Object>> getCart() {
		return SQLInterface.sqlExecute("select * from cart natural join cproduct where cid = '" + userInfo[0]+"';");
	}
	...
	private double countItems() {
		...
		//int count = 0;
		//for(int i = 1; i < cart.size();i++) {
		//	count += Integer.parseInt(cart.get(i).get(3).toString());
		//}
	...
	private void displayCheckoutInfo() {
		moneyDetails.setText("\n Subtotal                           $" + checkoutInfo[0] + "\n\n Delivery Fee                     $" + checkoutInfo[1] + "\n\n Tax                                   $" + checkoutInfo[2] + "\n\n Total                                $" + checkoutInfo[3]);
	}
