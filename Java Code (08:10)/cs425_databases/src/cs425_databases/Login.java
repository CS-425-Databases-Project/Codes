...
public class Login {
	...
	private boolean checkCredentials(String uname, String upass) {
		ArrayList<ArrayList<Object>> data = SQLInterface.sqlExecute("select uid from login where uname = '"+ uname + "' and upass = '" + upass + "';");
	...
	private boolean createAccount(String fname, String lname, String name, String pass) {
		...
		if(fname.equals("") || lname.equals("") || name.equals("") || pass.equals(""))
			...
			try {
				update1 = SQLInterface.sqlUpdate("insert into login values ('" + id + "', '" + 
						name + "', '" + pass +"');");
