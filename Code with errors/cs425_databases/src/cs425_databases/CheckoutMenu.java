...
public class CheckoutMenu {
	...
	private void getCards() {
		...
		ResultSet rs = database.sqlCode("select number from card;");
		try {
			while(rs.next()) {
		...
		for(int i = 0; i < cardNumbers.size(); i++) {
			existingCards.addItem(cardNumbers.get(i));
		}
	...
	private void getAddress() {
		...
		database.sqlCode("select name, street, city, state, zipcode from card where number = ?;", cardNumber); // change
	}
	...
	private void _countItems() {
		ResultSet rs = database.sqlCode("select count(*) from cart;");
	...
	private void createNewCard(String name, String number, String expiration) {
		...
		database.sqlCode("insert into card values ('?', '?', '?', '?')"); // change
	...
	private void cancelOrder() {
		String cart_id = "0"; // Put actual Cart Id
		
		database.sqlCode("delete from cart where (select * from cart where cart_id = ?);", cart_id);
		
		_message("Success!", "Your order has been cancelled successfully!", 0);
		fCheckout.setVisible(false);
		new Grocery_store(userInfo, cart);
	...
	private void placeOrder() {
		...
		if (!_checkSelectedCard()) {
			...
			database.sqlCode("insert into cart values ('?', '?', '?', '?');"); // change
